...
下一步
- 关卡之间能够切换：设想是关卡之间通过“门”来进行切换。角色走到靠关卡边缘的时候，镜头不会再随着角色移动而移动。当角色触碰到门之后，另外一遍的关卡被加载，然后镜头平滑移动到另外一个关卡。此时，之前的关卡依旧是在的，至少当需要展示关卡切换的时候，关卡还在。目的是实现一种类似“无缝加载”的错觉
    - 关卡切换，还要考虑到Entity的unload实现了！
    - 关卡切换，还要考虑时间回溯！比方说，切换关卡之后，时间回溯中存储的历史帧需要被清理掉。因为切换关卡，历史帧里面引用的内容全部会失效！

下一步
编写一个符合该项目规范的AGENTS.md文件

2026/02/01
- 增加了事件系统，用来在Manager、System之间广播消息。
- 增加了多继承辅助类，用来完善Lua中的多继承功能

下一步
增加一个时间变速的功能，玩家自己(及关联物体)和周围环境的流速可以不一样
- 本质上是需要将自身的时间和环境的时间流速解耦
- 时间回溯为了保持一致性，它也同时受时间变速功能的影响
- 物理系统的模拟，时间流速也要跟着一起调整？但Box2d本身没有区分时间步长这个功能

TODO:
BUG: 黑洞消失之后，存在部分物体的物理表现异常。具体而言是本该下坠，但是下坠速度比预期慢很多